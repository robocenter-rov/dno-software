...
float MS561101BA::getPressure(uint8_t OSR) {
  ...
  int64_t off  = (static_cast<uint32_t>(_C[1]) <<16) + ((static_cast<int64_t>(dT) * _C[3]) >> 7);
  int64_t sens = (static_cast<uint32_t>(_C[0]) <<15) + ((static_cast<int64_t>(dT) * _C[2]) >> 8);
...
int32_t MS561101BA::getDeltaTemp(uint8_t OSR) {
  ...
  if(rawTemp != NULL) {
    return static_cast<int32_t>(rawTemp - (static_cast<uint32_t>(_C[4]) << 8));
  }
  else {
    return NULL;
  }
...
uint32_t MS561101BA::getConversion(uint8_t command) {
  ...
  Wire.beginTransmission(_addr);
  Wire.requestFrom(_addr, static_cast<uint8_t>(MS561101BA_D1D2_SIZE));
  if(Wire.available()) {
...
int MS561101BA::readPROM() {
  for (int i=0;i<MS561101BA_PROM_REG_COUNT;i++) {
    ...
    Wire.beginTransmission(_addr);
    Wire.requestFrom(_addr, static_cast<uint8_t>(MS561101BA_PROM_REG_SIZE));
    if(Wire.available()) {
